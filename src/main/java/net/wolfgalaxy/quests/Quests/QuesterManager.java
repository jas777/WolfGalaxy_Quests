...
public class QuesterManager {
    ...
    public void registerNewQuester(String questerName, Location loc) {
        NPC npc = NPCManager.getManager().createNPC(questerName, loc, EntityType.PLAYER);
        Quester quester = new Quester(npc);
        if (quester == null) {
            ChatUtil.sendMessage(Bukkit.getConsoleSender(), "&cQuester creation failed.");
            return;
        }
        ...
        quester.saveToDataFile();
        questers.add(quester);
        ChatUtil.sendMessage(Bukkit.getConsoleSender(), "&aQuester created succesfully!");
    }
